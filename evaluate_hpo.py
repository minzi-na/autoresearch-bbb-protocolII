...
The std buffer was dropped (combo2 user preference, adopted here). std
stays in the TSV for reference only. Marginal wins (close to threshold)
should be cross-checked against the architecture_log holdout signal.

Usage:
    conda run -n rapids-25.02 python evaluate_hpo.py \\
        --combo maccs+avalon+scage2+mole --iter-id 1 --note "iter1 pilot"
...
BUDGET = {
    # Initial budget for combo1 phase-2 mirrors combo2's iter5-onward
    # budget (combo2 user-decided n_trials=30 from iter5 after observing
...
STORAGE_URL = "sqlite:///combo1_hpo.db"  # per-worktree DB; gitignored
