...
predictions on Holdout_nn05 (329) and write them to CSV.
...
Verification gates (must match the recorded iter198 numbers):
  ...
  - nn05 per-seed mean ROC == 0.8700 ,  soft-vote ensemble ROC == 0.8755
"""
import sys
import numpy as np
import pandas as pd
...
OUT = "/home/minji/BBB_paper/shared_splits/predictions/protocolII_best_holdout_nn05_predictions.csv"
...
for i, seed in enumerate(SEEDS):
    ...
    print(f"[{i+1:>2}/10] seed={seed:>3}  val_auc={val_metrics['roc_auc']:.6f}  nn05_roc={r:.6f}")
...
print(f"nn05 per-seed mean ROC : {ps_mean_roc:.6f}  (recorded 0.8700)")
print(f"nn05 ensemble ROC      : {ens_roc:.6f}  (recorded 0.8755)")
