...
class CrossModalFiLM(nn.Module):
    ...
    def forward(self, X):
        ...
        gf = F.dropout(gf, p=0.1, training=self.training)
        bf = F.dropout(bf, p=0.1, training=self.training)
        ge = F.dropout(ge, p=0.1, training=self.training)
        be = F.dropout(be, p=0.1, training=self.training)
