...
class MultiModalGMLPFromFlat(nn.Module):
    def __init__(self, mod_dims: OrderedDict, d_model=512, d_ffn=1024,
        ...
        self.mod_pos = nn.Parameter(torch.zeros(self.seq_len, d_model))
        nn.init.normal_(self.mod_pos, std=0.02)
        ...
        if use_gated_pool:
            ...
            self.pool_logits = nn.Parameter(torch.tensor([0.0, 0.0, 0.0, 1.0]))
