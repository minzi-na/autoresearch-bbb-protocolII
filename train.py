...
class MultiModalGMLPFromFlat(nn.Module):
    def __init__(self, mod_dims: OrderedDict, d_model=512, d_ffn=1024,
        ...
        if use_gated_pool:
            ...
            self.max_proj = nn.Linear(d_model, d_model)
            self.attn_head_proj = nn.Linear(d_model, d_model)
            for m in (self.gated_proj, self.mean_proj, self.max_proj, self.attn_head_proj):
    ...
    def forward(self, x):
        ...
        if self.use_gated_pool:
            ...
            gated = self.gated_proj(gated)
            mean = self.mean_proj(mean)
            max_pool = self.max_proj(max_pool)
