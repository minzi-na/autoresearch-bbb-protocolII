...
class MultiModalGMLPFromFlat(nn.Module):
    def __init__(self, mod_dims: OrderedDict, d_model=512, d_ffn=1024,
        ...
        self.adaln_norm = nn.LayerNorm(d_model, elementwise_affine=False)
        self.adaln_e2f = nn.Linear(d_model, d_model * 2)
        self.adaln_f2e = nn.Linear(d_model, d_model * 2)
        nn.init.zeros_(self.adaln_e2f.weight); nn.init.zeros_(self.adaln_e2f.bias)
        nn.init.zeros_(self.adaln_f2e.weight); nn.init.zeros_(self.adaln_f2e.bias)
    ...
    def forward(self, x):
        ...
        if len(self.fp_idx) > 0 and len(self.emb_idx) > 0:
            ...
            gf, bf = self.adaln_e2f(emb_sum).chunk(2, dim=-1)
            ge, be = self.adaln_f2e(fp_sum).chunk(2, dim=-1)
            X_norm = self.adaln_norm(X)
            X_film = X.clone()
            X_film[:, self.fp_idx,  :] = (1 + gf.unsqueeze(1)) * X_norm[:, self.fp_idx,  :] + bf.unsqueeze(1)
            X_film[:, self.emb_idx, :] = (1 + ge.unsqueeze(1)) * X_norm[:, self.emb_idx, :] + be.unsqueeze(1)
            X = X_film
        X = self.backbone(X)
        if self.use_gated_pool:
            w = torch.softmax(self.alpha, dim=0)
            Xp_gated = (X * w.view(1, -1, 1)).sum(dim=1)
            Xp_mean = X.mean(dim=1)
            g = torch.sigmoid(self.skip_gate)
            Xp = g * Xp_gated + (1 - g) * Xp_mean
        else:
            Xp = X.mean(dim=1)
